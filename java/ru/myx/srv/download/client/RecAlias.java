...
public final class RecAlias {
	 ...
	 * @throws Exception */
	public final void doUpdateConnections(final Collection<String> addFollowLinks, final Collection<String> removeFollowLinks) throws Exception {

		if ((addFollowLinks == null || addFollowLinks.isEmpty()) && (removeFollowLinks == null || removeFollowLinks.isEmpty())) {
			return;
		}
			...
			try {
				...
				if (removeFollowLinks != null && !removeFollowLinks.isEmpty()) {
					try (final PreparedStatement ps = conn.prepareStatement( //
							"DELETE FROM d1ItemLinkage " + //
									"WHERE itmGuid in (" + //
									"SELECT i.itmGuid FROM d1Items i, d1Known k, d1KnownAliases a WHERE a.guid=k.guid and a.alias=i.itmLevel1Name and a.alias=?" + //
									") AND itmLink IN ('" + String.join("','", removeFollowLinks) + "')" //
					)) {
						ps.setString(1, this.alias);
						ps.execute();
					}
					try (final PreparedStatement ps = conn.prepareStatement(//
							"DELETE FROM d1KnownAliases " + //
									"WHERE alias=? AND guid IN ('" + String.join("','", removeFollowLinks) + "')"//
					)) {
						ps.setString(1, this.alias);
						ps.execute();
					}
				...
				if (addFollowLinks != null && !addFollowLinks.isEmpty()) {
					for (final String followLink : addFollowLinks) {
						...
						try (final PreparedStatement ps = conn.prepareStatement("DELETE FROM d1KnownAliases WHERE alias=? AND guid=?")) {
							ps.setString(1, this.alias);
							ps.setString(2, followLink);
							ps.execute();
						}
						try (final PreparedStatement ps = conn.prepareStatement("INSERT INTO d1KnownAliases(alias,guid) VALUES (?, ?)")) {
							ps.setString(1, this.alias);
							ps.setString(2, followLink);
							ps.execute();
						}
					...
					try (final PreparedStatement ps = conn.prepareStatement( //
							"DELETE FROM d1ItemLinkage " + //
									"WHERE itmGuid in (" + //
									"SELECT i.itmGuid FROM d1Items i, d1Known k, d1KnownAliases a WHERE a.guid=k.guid and a.alias=i.itmLevel1Name and a.alias=?" + //
									") AND itmLink IN ('" + String.join("','", addFollowLinks) + "')"//
					)) {
						ps.setString(1, this.alias);
						ps.execute();
					}
					try (final PreparedStatement ps = conn.prepareStatement( //
							"INSERT INTO d1ItemLinkage(itmGuid,itmLink) " + //
									"SELECT i.itmGuid,k.guid FROM d1Items i, d1Known k, d1KnownAliases a " + //
									"WHERE a.guid=k.guid and a.alias=i.itmLevel1Name and a.alias=? AND k.guid IN ('" + String.join("','", addFollowLinks) + "')"//
					)) {
						ps.setString(1, this.alias);
						ps.execute();
					}
