...
public class RecFile {
	...
	public static final NameParser checkParser(final String name, final RecFile file) {
		...
		return parser.getLevel2Name().equals( file.getLevel2Name() )
				&& parser.getLevel3Name().equals( file.getLevel3Name() )
				? null
				: parser;
	}
	...
	public static final void create(
		...
		final NameParser parser = NameParser.createNameParser( searchName, fldLuid, parent );
		{
			try (final PreparedStatement ps = conn
					.prepareStatement( "INSERT INTO d1Items(itmGuid,itmCrc,fldLuid,itmName,itmCreated,itmSize,itmDate,itmType,itmComment,itmPreview,itmSearchName,itmLevel1Name,itmLevel2Name,itmLevel3Name) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?)" )) {
				ps.setString( 1, guid );
				ps.setString( 2, md5 );
				ps.setInt( 3, fldLuid );
				ps.setString( 4, name );
		...
		if (parser != null) {
			...
			if (follow != null && !follow.isEmpty()) {
				try (final PreparedStatement ps = conn
						.prepareStatement( "INSERT INTO d1ItemLinkage(itmGuid,itmLink) VALUES (?,?)" )) {
	...
	private static final Set<String> insertFollowIdentifier(final Connection conn, final String alias, final String hint)
			throws SQLException {
		...
		try (final PreparedStatement ps = conn.prepareStatement( "SELECT queLuid FROM d1Queue WHERE queFormation=?",
				ResultSet.TYPE_FORWARD_ONLY,
				ResultSet.CONCUR_READ_ONLY )) {
			ps.setString( 1, alias.toLowerCase() );
			try (final ResultSet rs = ps.executeQuery()) {
		...
		try (final PreparedStatement ps = conn
			...
			ps.setString( 5, hint == null || hint.length() == 0
					? "*"
					: hint );
			ps.execute();
			return null;
	 ...
	 * @throws SQLException
	 */
	public static void remove(final Connection conn, final DownloadClient client, final int fldLuid, final String name)
			throws SQLException {
		{
	...
	RecFile(final DownloadClient client,
			final String guid,
			final int itmLuid,
			final int fldLuid,
			final String name,
	...
	public final Collection<String> getAliases() throws Exception {
		...
		try {
			final PreparedStatement ps = conn
					.prepareStatement( "SELECT a.guid FROM d1KnownAliases a, d1Items i WHERE a.alias=i.itmLevel1Name AND i.itmCrc=?",
							ResultSet.TYPE_FORWARD_ONLY,
							ResultSet.CONCUR_READ_ONLY );
			try {
	...
	public String getDescription(final Connection conn) throws Exception {
		...
		if (this.description == null) {
			synchronized (this) {
				if (this.description == null) {
					try (final PreparedStatement ps = conn
							.prepareStatement( "SELECT itmDescription,itmHidden FROM d1Descriptions WHERE itmCrc=?",
									ResultSet.TYPE_FORWARD_ONLY,
									ResultSet.CONCUR_READ_ONLY )) {
						ps.setString( 1, this.md5 );
						try (final ResultSet rs = ps.executeQuery()) {
			...
			synchronized (this) {
				if (this.hidden == -1) {
					try (final Connection conn = this.client.nextConnection()) {
						...
						try (final PreparedStatement ps = conn
								.prepareStatement( "SELECT itmHidden FROM d1Descriptions WHERE itmCrc=?",
										ResultSet.TYPE_FORWARD_ONLY,
										ResultSet.CONCUR_READ_ONLY )) {
							ps.setString( 1, this.md5 );
							try (final ResultSet rs = ps.executeQuery()) {
	...
	public boolean update(
			...
			final long date,
			final boolean preview,
			final NameParser parser) throws SQLException {
		int changes = 0;
		...
		if (date / 10000L != this.date / 10000L) {
			if (changes++ > 0) {
				query.append( ", " );
			}
