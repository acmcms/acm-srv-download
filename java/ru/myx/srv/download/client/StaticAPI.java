...
public class StaticAPI {
	...
	public void ensureKnown(final String name) throws Exception {
		...
		try (final Connection conn = this.client.nextConnection()) {
			...
			conn.setAutoCommit(false);
			{
				...
				{
					try (final PreparedStatement ps = conn.prepareStatement("SELECT count(*) FROM d1Known WHERE name=?", ResultSet.TYPE_FORWARD_ONLY, ResultSet.CONCUR_READ_ONLY)) {
						ps.setString(1, name);
				...
				final boolean aliasFound;
				{
					try (final PreparedStatement ps = conn
							.prepareStatement("SELECT count(*) FROM d1KnownAliases WHERE alias=?", ResultSet.TYPE_FORWARD_ONLY, ResultSet.CONCUR_READ_ONLY)) {
				...
				if (!knownFound && !aliasFound) {
					...
					}
					{
						final int count;
						try (final PreparedStatement ps = conn
								.prepareStatement("SELECT count(*) FROM d1Queue WHERE queFormation=?", ResultSet.TYPE_FORWARD_ONLY, ResultSet.CONCUR_READ_ONLY)) {
							ps.setString(1, name.toLowerCase());
							try (final ResultSet rs = ps.executeQuery()) {
								if (rs.next()) {
									count = rs.getInt(1);
	...
	public int getL1KnownAliasCount() throws SQLException {
		...
		try (final Connection conn = this.client.nextConnection()) {
			...
			try (final PreparedStatement ps = conn.prepareStatement("SELECT count(*) FROM d1KnownAliases", ResultSet.TYPE_FORWARD_ONLY, ResultSet.CONCUR_READ_ONLY)) {
				try (final ResultSet rs = ps.executeQuery()) {
					if (rs.next()) {
						return rs.getInt(1);
					}
	...
	public int getL1KnownCount() throws SQLException {
		...
		try (final Connection conn = this.client.nextConnection()) {
			...
			try (final PreparedStatement ps = conn.prepareStatement("SELECT count(*) FROM d1Known", ResultSet.TYPE_FORWARD_ONLY, ResultSet.CONCUR_READ_ONLY)) {
				try (final ResultSet rs = ps.executeQuery()) {
					if (rs.next()) {
						return rs.getInt(1);
					}
	...
	public int getL1UnknownCount() throws SQLException {
		...
		try (final Connection conn = this.client.nextConnection()) {
			...
			try (final PreparedStatement ps = conn.prepareStatement("SELECT count(*) FROM d1Queue", ResultSet.TYPE_FORWARD_ONLY, ResultSet.CONCUR_READ_ONLY)) {
				try (final ResultSet rs = ps.executeQuery()) {
					if (rs.next()) {
						return rs.getInt(1);
					}
	...
	public int getNextQueued() throws SQLException {
		...
		try (final Connection conn = this.client.nextConnection()) {
			...
			try (final PreparedStatement ps = conn.prepareStatement(
					"SELECT MIN(q.queLuid),count(*) FROM d1Queue q, d1Items i WHERE q.queBusy<? AND q.queFormation=i.itmLevel1Name GROUP BY q.queFormation, q.queDelay ORDER BY q.queDelay, 2 DESC",
		...
		try (final Connection conn = this.client.nextConnection()) {
			...
			try (final PreparedStatement ps = conn.prepareStatement(
				...
				try (final ResultSet rs = ps.executeQuery()) {
					...
					while (rs.next()) {
						result.add(new RecQueued(this.client, rs.getInt(1), rs.getString(2).toLowerCase(), rs.getBytes(3), rs.getString(4)));
					}
	...
	public RecQueued getQueued(final int luid) throws Exception {
		...
		try (final Connection conn = this.client.nextConnection()) {
			...
			try (final PreparedStatement ps = conn
					.prepareStatement("SELECT queFormation,queText,queHint FROM d1Queue WHERE queLuid=?", ResultSet.TYPE_FORWARD_ONLY, ResultSet.CONCUR_READ_ONLY)) {
				ps.setMaxRows(1);
				ps.setInt(1, luid);
				try (final ResultSet rs = ps.executeQuery()) {
					if (rs.next()) {
						return new RecQueued(this.client, luid, rs.getString(1).toLowerCase(), rs.getBytes(2), rs.getString(3));
					}
					return null;
				}
			}
	...
	public void setItemDescription(final String md5, final String description, final boolean hidden) throws Exception {
		...
		try (final Connection conn = this.client.nextConnection()) {
			...
			if (realDescription.length() > 0 || hidden) {
				try (final PreparedStatement ps = conn.prepareStatement("INSERT INTO d1Descriptions(itmCrc,itmDescription,itmHidden) VALUES (?,?,?)")) {
					ps.setString(1, md5);
	...
	public void setQueued(final String name, final String text) throws Exception {
		...
		try (final Connection conn = this.client.nextConnection()) {
			...
			final int count;
			try (final PreparedStatement ps = conn.prepareStatement("SELECT count(*) FROM d1Queue WHERE queFormation=?", ResultSet.TYPE_FORWARD_ONLY, ResultSet.CONCUR_READ_ONLY)) {
				ps.setString(1, name.toLowerCase());
				try (final ResultSet rs = ps.executeQuery()) {
					if (rs.next()) {
						count = rs.getInt(1);
