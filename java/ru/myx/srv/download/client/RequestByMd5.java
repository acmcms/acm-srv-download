 ...
 *
 */
final class RequestByMd5 extends RequestAttachment<RecFile[], RunnerDatabaseRequestor> {
	...
	RequestByMd5(final DownloadClient parent, final String key, final String md5, final String typeFilter, final boolean all) {
		this.parent = parent;
		this.key = key;
		this.md5 = md5;
		...
		query.append("SELECT i.itmGuid,i.itmCrc,i.itmLuid,i.fldLuid,i.itmName,i.itmSize,i.itmDate,i.itmType,i.itmComment,i.itmPreview,i.itmLevel2Name,i.itmLevel3Name ")
				.append("FROM d1Items i, d1Folders f, d1Sources s WHERE i.fldLuid=f.fldLuid AND f.srcLuid=s.srcLuid AND i.itmCrc=?");
		...
		try (final PreparedStatement ps = ctx.ctxGetConnection().prepareStatement(this.query, ResultSet.TYPE_FORWARD_ONLY, ResultSet.CONCUR_READ_ONLY)) {
			...
			try (final ResultSet rs = ps.executeQuery()) {
				while (rs.next()) {
					...
					pending.add(new RecFile(this.parent, guid, itmLuid, parentLuid, name, md5, size, date, type, comment, preview, level2Name, level3Name, null));
				}
			}
