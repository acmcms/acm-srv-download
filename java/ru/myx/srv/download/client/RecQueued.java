...
public final class RecQueued {
	...
	RecQueued(final DownloadClient client, final int luid, final String formation, final byte[] text, final String hint) throws Exception {
	...
	public void doAcknowledge(final String followLink, final Collection<String> names) throws SQLException {
		...
		try (final Connection conn = this.client.nextConnection()) {
			...
			try (final PreparedStatement ps = conn.prepareStatement("INSERT INTO d1KnownAliases(guid,alias) VALUES (?,?)")) {
				ps.setString(1, followLink);
				ps.setString(2, this.formation.toLowerCase());
				ps.execute();
			}
			try (final PreparedStatement ps = conn.prepareStatement(
					"INSERT INTO d1ItemLinkage(itmGuid,itmLink) SELECT i.itmGuid,? FROM d1Items i LEFT OUTER JOIN d1ItemLinkage l ON i.itmGuid=l.itmGuid AND l.itmLink=? WHERE i.itmLevel1Name=? AND l.itmGuid is NULL")) {
				ps.setString(1, followLink);
			...
			for (final String followLink : followLinks) {
				{
					...
					try (final PreparedStatement ps = conn.prepareStatement("SELECT count(*) FROM d1Known WHERE guid=?", ResultSet.TYPE_FORWARD_ONLY, ResultSet.CONCUR_READ_ONLY)) {
						ps.setString(1, followLink);
				...
				}
				{
					...
					try (final PreparedStatement ps = conn
							.prepareStatement("SELECT count(*) FROM d1KnownAliases WHERE guid=? AND alias=?", ResultSet.TYPE_FORWARD_ONLY, ResultSet.CONCUR_READ_ONLY)) {
						ps.setString(1, followLink);
	...
	public void doCreateAlias(final String followLink) throws SQLException {
		...
		try (final Connection conn = this.client.nextConnection()) {
			...
			try (final PreparedStatement ps = conn.prepareStatement(
					"INSERT INTO d1ItemLinkage(itmGuid,itmLink) SELECT i.itmGuid,? FROM d1Items i LEFT OUTER JOIN d1ItemLinkage l ON i.itmGuid=l.itmGuid WHERE l.itmGuid is NULL AND i.itmLevel1Name=?")) {
				ps.setString(1, followLink);
				ps.setString(2, this.formation);
				ps.execute();
			}
	...
	public void doDivide(final Collection<String> names) throws SQLException {
		...
		try (final Connection conn = this.client.nextConnection()) {
			...
			for (final String name : names) {
				...
				{
					try (final PreparedStatement ps = conn.prepareStatement(
							"SELECT guid FROM d1Known WHERE name=? UNION SELECT guid FROM d1KnownAliases WHERE alias=?",
	...
	public void doLock() throws SQLException {
		...
		try (final Connection conn = this.client.nextConnection()) {
			...
			try (final PreparedStatement ps = conn.prepareStatement("UPDATE d1Queue SET queBusy=? WHERE queLuid=? AND queFormation=?")) {
				ps.setMaxRows(1);
	...
	public void doRequeue() throws SQLException {
		...
		try (final Connection conn = this.client.nextConnection()) {
			...
			final long last;
			try (final PreparedStatement ps = conn.prepareStatement("SELECT MAX(queQueued) FROM d1Queue", ResultSet.TYPE_FORWARD_ONLY, ResultSet.CONCUR_READ_ONLY)) {
				try (final ResultSet rs = ps.executeQuery()) {
					if (rs.next()) {
			...
			try (final PreparedStatement ps = conn.prepareStatement("UPDATE d1Queue SET queQueued=?, queDelay=queDelay+1 WHERE queLuid=? AND queFormation=?")) {
				ps.setTimestamp(1, new Timestamp(last + 60_000L));
				ps.setInt(2, this.luid);
				ps.setString(3, this.formation.toLowerCase());
				ps.execute();
			}
