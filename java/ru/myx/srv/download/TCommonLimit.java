...
final class TCommonLimit implements Runnable, Traffic {
	...
	TCommonLimit(final String name, final TrafficFilter parent, final CounterBlockFilter counter, final BaseObject data) {
		...
		}
		{
			final char direction = value == lastValue
				...
				: value < lastValue
					? '-'
					: '+';
			Report.info("SHAPER", this.description + ", shape(" + direction + "):\t ttl=" + Format.Compact.toBytes(this.limit)
			// + "B\t eff="
			// + Format.Compact.toBytes(limitK)
			// + "B\t per="
			// + Format.Compact.toBytes(value)
					+ "B\t cnt=" + load + "/" + this.counter.getValueAll() + "\t sm=" + Format.Compact.toBytes(speed) + " \t sa="
