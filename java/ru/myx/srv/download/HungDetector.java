...
class HungDetector implements Runnable {
	...
	public void run() {
		if (!this.done) {
			if (this.first) {
				Report.warning( "DLS/SHARE/HD",
						"Hung preview detected, wait another 60 seconds. Target=" + this.target.getAbsolutePath() );
				this.first = false;
				...
				Act.later( null, HungDetector.RESTARTER, 10000L );
			}
		}
	}
}
