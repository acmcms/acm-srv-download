 ...
 * <p>
 * The second option is to call com.twmacinta.util.MD5.initNativeLibrary(false)
...
class MD5 {
	...
	private static void doDecode(final byte buffer[], final int shift, final int[] out) {
		/*
	...
	private static byte[] doEncode(final int input[], final int len) {
		int i, j;
		byte out[];
		out = new byte[len];
	...
	private static void doTransform(final MD5State state, final byte buffer[], final int shift, final int[] decode_buf) {
		int a = state.state[0], b = state.state[1], c = state.state[2], d = state.state[3];
		final int x[] = decode_buf;
		MD5.doDecode( buffer, shift, decode_buf );
		/* Round 1 */
	...
	public synchronized byte[] doFinal() {
		...
		if (this.finals == null) {
			...
			bits = MD5.doEncode( count_ints, 8 );
			index = (int) (fin.count & 0x3f);
			padlen = index < 56
					? 56 - index
					: 120 - index;
			this.doUpdate( fin, MD5.padding, 0, padlen );
			this.doUpdate( fin, bits, 0, 8 );
			/* Update() sets finals to null */
			this.finals = fin;
		}
		return MD5.doEncode( this.finals.state, 16 );
	}
	 ...
	 */
	public void doUpdate(final byte buffer[]) {
		this.doUpdate( buffer, 0, buffer.length );
	}
	 ...
	 */
	public void doUpdate(final byte b) {
		final byte buffer[] = new byte[1];
		buffer[0] = b;
		this.doUpdate( buffer, 1 );
	}
	 ...
	 * @param length
	 */
	public void doUpdate(final byte buffer[], final int length) {
		this.doUpdate( this.state, buffer, 0, length );
	}
	 ...
	 * @param length
	 */
	public void doUpdate(final byte buffer[], final int offset, final int length) {
		this.doUpdate( this.state, buffer, offset, length );
	}
	 ...
	 *            buffer.length)
	 */
	public void doUpdate(final MD5State stat, final byte buffer[], final int offset, int length) {
		...
		if (length >= partlen) {
			...
			if (partlen == 64) {
				...
				MD5.doTransform( stat, stat.buffer, 0, decode_buf );
			}
			for (i = partlen; i + 63 < length; i += 64) {
				MD5.doTransform( stat, buffer, i + offset, decode_buf );
			}
			index = 0;
		} else {
			i = 0;
	 ...
	 */
	public void doUpdate(final String s) {
		final byte chars[] = s.getBytes();
		this.doUpdate( chars, chars.length );
	}
	 ...
	 */
	public void doUpdate(final String s, String charset_name) throws java.io.UnsupportedEncodingException {
